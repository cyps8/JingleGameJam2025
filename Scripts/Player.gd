...
var punchCost: float = 20
var blockCost: float = 10
...
func _process(_dt):
    ...
    if left && !usingL:
        if Input.is_action_just_pressed("punch") && stamCur > punchCost:
            ...
            recoverycd = 0.25
            usingL = true
        elif Input.is_action_just_pressed("block") && stamCur > blockCost:
            blockingL = true
